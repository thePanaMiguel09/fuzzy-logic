...
def plot_membership_functions():
    ...
    fig.text(
        0.5,
        0.97,
        "FUNCIONES DE MEMBRESÍA — SISTEMA DIFUSO ESFUERZO FÍSICO",
        ha="center",
        va="top",
        fontsize=15,
        fontweight="bold",
    ...
    for name, mf in fc_sets.items():
        ax1.plot(x_fc, mf, color=SET_COLORS[name], linewidth=2.5, label=labels_fc[name])
    ...
    for name, mf in tr_sets.items():
        ax2.plot(x_tr, mf, color=SET_COLORS[name], linewidth=2.5, label=labels_tr[name])
        ax2.fill_between(x_tr, mf, alpha=0.12, color=SET_COLORS[name])
    ax2.set_title(
        "Tasa Respiratoria (TR)",
        fontsize=12,
        fontweight="bold",
    ...
    for name, mf in pse_sets.items():
        ax3.plot(
            x_pse, mf, color=SET_COLORS[name], linewidth=2.5, label=labels_pse[name]
        )
        ax3.fill_between(x_pse, mf, alpha=0.12, color=SET_COLORS[name])
    ax3.set_title(
        "Percepción Subjetiva de Esfuerzo — PSE (Borg)",
        fontsize=12,
        fontweight="bold",
    ...
    ax4.set_title(
        "Salida — Intensidad del Esfuerzo (Mamdani)",
        fontsize=12,
        fontweight="bold",
...
def get_reglas(porcentaje_quitar=0):
    """
    Retorna la base de reglas según el % a quitar, distribuyendo
    la selección para cubrir todo el universo de discurso.
    """
    n_total = len(REGLAS_FULL)
    n_usar = max(1, int(n_total * (1 - porcentaje_quitar)))

    if porcentaje_quitar == 0:
        reglas = REGLAS_FULL
    else:
        # np.linspace selecciona índices distribuidos equitativamente
        # evitando que el sistema pierda el conocimiento de los extremos
        indices = np.linspace(0, n_total - 1, n_usar, dtype=int)
        reglas = [REGLAS_FULL[i] for i in indices]
...
def mamdani_inferencia(fc_val, tr_val, pse_val, reglas):
    ...
    for fc_l, tr_l, pse_l, sal_l, _ in reglas:
        alpha = min(fc_fuzz.get(fc_l, 0), tr_fuzz.get(tr_l, 0), pse_fuzz.get(pse_l, 0))
        if alpha > 0:
...
def mamdani_batch(casos, reglas):
    ...
    for fc, tr, pse in casos:
        ...
        resultados.append(
            {"FC": fc, "TR": tr, "PSE": pse, "salida": val, "agregado": agg}
        )
    return resultados
...
def plot_defuzz_mamdani(
    ...
    ax.fill_between(
        x_out, agregado, alpha=0.55, color=PALETTE["accent1"], label="Área agregada"
    )
    ...
    if salida_val is not None:
        ax.axvline(
            ...
            linewidth=2.5,
            ls="-",
            label=f"Centroide: {salida_val:.1f}%",
        )

    ax.set_xlim(0, 100)
    ax.set_ylim(0, 1.05)
    ax.set_title(
        caso_nombre, fontsize=9.5, fontweight="bold", color=PALETTE["text"], pad=5
    )
...
def run_all(escenarios_input=None):
    ...
    fig_res.suptitle(
        f"DEFUZZIFICACIÓN MAMDANI — {n_esc} ESCENARIO(S) × 3 CONFIGURACIONES DE REGLAS",
        fontsize=13,
        fontweight="bold",
        color=PALETTE["accent1"],
        y=0.98,
    ...
    for col_idx, (cfg_nombre, pct_quitar, n_reglas) in enumerate(CONFIGS_ESCENARIO):
        ...
        for i, ((esc_name, fc, tr, pse), mr, sr) in enumerate(
            ...
            print(f"  │  Caso {i+1}: {esc_name:<22} " f"FC={fc} TR={tr} PSE={pse}")
            ...
            plot_defuzz_mamdani(titulo, fc, tr, pse, mr["agregado"], mv, i, col_idx, ax)
        print(f"  └{'─'*55}")
...
def plot_comparacion(tabla_global, escenarios_input=None):
    ...
    fig.patch.set_facecolor(PALETTE["bg"])
    fig.suptitle(
        "COMPARACIÓN MAMDANI vs SUGENO — TRES CONFIGURACIONES DE REGLAS",
        fontsize=13,
        fontweight="bold",
        color=PALETTE["accent1"],
        y=1.02,
    ...
    for col_idx, cfg_c in enumerate(cfg_nombres_cortos):
        ...
        ax.set_xticks(x_pos)
        ax.set_xticklabels([e.replace(" ", "\n") for e in esc_nombres], fontsize=8.5)
        ax.set_ylim(0, 115)
        ax.set_ylabel("% Intensidad", fontsize=10)
        ax.set_title(
            cfg_c, fontsize=11, fontweight="bold", color=PALETTE["accent4"], pad=8
        )
...
def imprimir_tabla_resumen(tabla_global):
    ...
    for r in tabla_global:
        ...
        print(
            f"  {r['Escenario']:<22} {cfg_s:<22} {mv:>9} {mel:<12} {sv:>9} {sel:<12} {dv:>8}"
        )
    print("─" * 90)
...
print("""
  ...
  ¿Resultados similares?
    Sí. Con el 100% de reglas, Mamdani y Sugeno convergen con una diferencia 
    máxima aproximada del 7% en los escenarios evaluados. Las diferencias 
    son razonables y se mantienen estables gracias a la coherencia en la 
    base de reglas.

  ¿Dónde se observan mayores diferencias o problemas?
    Cuando se reduce drásticamente el número de reglas (ej. al 30%) dejando 
    "huecos" de conocimiento, el sistema sufre una falta de activación 
    (arroja N/A). Esto demuestra empíricamente que un sistema difuso requiere 
    una base de reglas que cubra adecuadamente todo el universo de discurso.
    Por otro lado, cuando las reglas se distribuyen equitativamente, Sugeno 
    tiende a ser más sensible a los valores extremos que Mamdani.
...
def imprimir_tabla_escenarios(escenarios, descripciones):
    ...
    print("═" * ancho)
    print(
        f"  {'N°':<4} {'Escenario':<24} {'FC (ppm)':>9} {'TR (rpm)':>9} {'PSE (Borg)':>11}  Descripción"
    )
    print("  " + "─" * (ancho - 2))
    for i, ((nombre, fc, tr, pse), desc) in enumerate(
        zip(escenarios, descripciones), 1
    ):
    ...
    while True:
        try:
            ...
            if minimo <= val <= maximo:
                return val
            print(
                f"    ⚠ El valor debe estar entre {minimo} y {maximo}. Intenta de nuevo."
            )
        except ValueError:
